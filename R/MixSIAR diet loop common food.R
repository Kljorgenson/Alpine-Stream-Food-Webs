...
iso_means <- dat_3 %>%  dplyr::group_by(group, site) %>% dplyr::filter(date2 == "A", !site %in% "Gusher") %>%
  dplyr::summarise( type = type,
    n = length(group),
...
iso_means %>% ggplot(aes(Meand13C, Meand15N, color = type)) + geom_point()
